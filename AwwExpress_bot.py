...
def vreddit(bot, update):
    ...
    #youtube-dl
    dlprocess = subprocess.Popen(['youtube-dl', redditurl, '--restrict-filenames'], stdout=PIPE, stderr=STDOUT, bufsize=1, universal_newlines=True)
    dlprocess.wait()
    ...
    nameprocess = subprocess.Popen(['youtube-dl', redditurl, '--no-warnings', '--restrict-filenames', '--get-filename'], stdout=PIPE, stderr=STDOUT, bufsize=1, universal_newlines=True)
    youtubeoutputstring = ""
    for line in nameprocess.stdout:
        print(line, end='') 
        youtubeoutputstring+=str(line)
    nameprocess.wait()
    youtubeoutputstring = str(youtubeoutputstring).decode('utf8')
